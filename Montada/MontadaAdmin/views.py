...
def _admin_enrich_market_row(row, default_round_digits=4):
    """
    Add computed fields using mt5_prices field names: AskLast, BidLast, AskLow, BidLow,
    Digits, etc. Adds: dir, ask_today, bid_today, change, change_percentage.
    Same logic as Dashboard GetMarketDataFromMT5.
    """
    ask_current_val = _admin_row_val(row, 'AskLast')
    bid_current_val = _admin_row_val(row, 'BidLast')
...
class AdminGetMarketDataFromMT5(APIView):
    ...
    Example: GET /admin/marketdata/live?category=forex
    """
    permission_classes = [IsAuthenticated, IsAdminUser]
